...
/**
 * Teacher Import Class
 * 
 * Handles Excel import for Teacher data
 */
class TeacherImport implements ToModel, WithHeadingRow, WithValidation, WithChunkReading, WithEvents
{
    ...
    public function model(array $row)
    {
        try {
            ...
            // Create teacher
            $teacher = $this->teacherService->create($this->prepareData($row));
            $this->successCount++;
    ...
    {
        $query = Teacher::where('instansi_id', $this->teacherService->repository->currentTenantId);

        if (!empty($row['nuptk'])) {
            $query->where('nuptk', $row['nuptk']);
        }
    ...
    public function registerEvents(): array
    {
        return [
            ...
            ImportFailed::class => function(ImportFailed $event) {
                ...
                \Log::error('Teacher import failed: ' . $event->getException()->getMessage());
            },
        ];
    }
