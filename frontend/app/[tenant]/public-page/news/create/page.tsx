...
export default function CreateNewsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormData) => {
      return publicPageAdminApi.createNews(tenantId!, data);
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      showError(err?.response?.data?.message || 'Gagal membuat berita');
    },
  });
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 border border-gray-100 dark:border-slate-700 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Masukkan konten berita (HTML diperbolehkan)"
            />
          </div>
