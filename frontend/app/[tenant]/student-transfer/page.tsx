      ...
      <div className="p-6 bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30 min-h-screen">
                ...
                <TableBody>
                  {filteredData.map((transfer) => (
                    <TableRow key={transfer.id} className="hover:bg-blue-50/50 transition-colors">
                      <TableCell className="font-medium text-gray-800">{transfer.student_name || '-'}</TableCell>
        ...
        >
          <form onSubmit={(e) => {
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Ke Instansi <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                value={formData.to_instansi_id}
                onChange={(e) => setFormData({ ...formData, to_instansi_id: parseInt(e.target.value) || 0 })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
                min="1"
              />
