...
export default function AnnouncementPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: AnnouncementCreateData) => {
      if (!tenantId) {
        throw new Error('Tenant ID tidak tersedia.');
      }
      return announcementApi.create(tenantId, data);
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<AnnouncementCreateData> }) => {
      ...
      return announcementApi.update(tenantId, id, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['announcements', tenantId] });
      setIsModalOpen(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => {
      ...
      return announcementApi.delete(tenantId, id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['announcements', tenantId] });
    },
  ...
  return (
    <TenantLayout>
      <ModulePageShell
        ...
        title="Pengumuman"
        description="Kelola pengumuman untuk instansi Anda"
        actions={({ themeConfig }) => (
          <Button
            onClick={() => {
              resetForm();
          ...
          <>
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, priority: e.target.value as AnnouncementCreateData['priority'] })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, status: e.target.value as AnnouncementCreateData['status'] })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="draft">Draft</option>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Target Audien</label>
                  <select
                    value={formData.target_audience}
                    onChange={(e) =>
                      setFormData({ ...formData, target_audience: e.target.value as AnnouncementCreateData['target_audience'] })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
