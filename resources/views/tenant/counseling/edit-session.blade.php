    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="page-header">
                ...
                <p class="text-muted mb-0 mt-2" style="font-size: 1.05rem;">Perbarui informasi sesi konseling</p>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="form-card">
                ...
                <form action="{{ tenant_route('counseling.sessions.update', $session->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body p-4">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="counselor_id" id="counselor_id" class="form-select @error('counselor_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($counselors as $counselor)
                                        <option value="{{ $counselor->id }}" {{ old('counselor_id', $session->counselor_id) == $counselor->id ? 'selected' : '' }}>
                                            {{ $counselor->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="session_date" id="session_date" 
                                       ...
                                       value="{{ old('session_date', $session->session_date ? $session->session_date->format('Y-m-d') : '') }}" required>
                                @error('session_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="session_type" id="session_type" class="form-select @error('session_type') is-invalid @enderror" required>
                                    ...
                                    <option value="family" {{ old('session_type', $session->session_type) == 'family' ? 'selected' : '' }}>Keluarga</option>
                                    <option value="crisis" {{ old('session_type', $session->session_type) == 'crisis' ? 'selected' : '' }}>Krisis</option>
                                </select>
                                @error('session_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="category" id="category" class="form-select @error('category') is-invalid @enderror" required>
                                    ...
                                    <option value="academic" {{ old('category', $session->category) == 'academic' ? 'selected' : '' }}>Akademik</option>
                                    <option value="behavioral" {{ old('category', $session->category) == 'behavioral' ? 'selected' : '' }}>Perilaku</option>
                                    <option value="emotional" {{ old('category', $session->category) == 'emotional' ? 'selected' : '' }}>Emosional</option>
                                    <option value="social" {{ old('category', $session->category) == 'social' ? 'selected' : '' }}>Sosial</option>
                                    <option value="career" {{ old('category', $session->category) == 'career' ? 'selected' : '' }}>Karir</option>
                                    <option value="personal" {{ old('category', $session->category) == 'personal' ? 'selected' : '' }}>Personal</option>
                                </select>
                                @error('category')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                                    ...
                                    <option value="in_progress" {{ old('status', $session->status) == 'in_progress' ? 'selected' : '' }}>Sedang Berlangsung</option>
                                    ...
                                    <option value="no_show" {{ old('status', $session->status) == 'no_show' ? 'selected' : '' }}>Tidak Hadir</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="goals" id="goals" 
                                      ...
                                      rows="3" 
                                      placeholder="Tentukan tujuan yang ingin dicapai melalui konseling (opsional)">{{ old('goals', is_array($session->goals) ? implode(', ', $session->goals) : $session->goals) }}</textarea>
                            @error('goals')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="notes" id="notes" 
                                      ...
                                      rows="3" 
                                      placeholder="Catatan tambahan (opsional)">{{ old('notes', $session->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
